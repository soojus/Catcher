...
/* USER CODE BEGIN 4 */
#define FRAME_BUFFER_A  0xC0000000
#define FRAME_BUFFER_B  0xC003FC00
#define FRAME_BUFFER_C  0xC007F800
#define FRAME_BUFFER_D  0xC00BF400
#define OV5640_I2C_ADDRESS 0x78
uint32_t cam_buf[256] ={0};
uint32_t width = 480;
uint32_t height = 272;
uint32_t couta = 0;
uint32_t coutb = 0;
...
void XferCpltCallback(DMA_HandleTypeDef *hdma)
{
	...
	HAL_DMAEx_ChangeMemory(hdma, FRAME_BUFFER_A, MEMORY0);
	couta++;

}
void XferM1CpltCallback(DMA_HandleTypeDef *hdma)
{
	HAL_LTDC_SetAddress(&hltdc, FRAME_BUFFER_A,0);
	HAL_DMAEx_ChangeMemory(hdma, FRAME_BUFFER_B, MEMORY1);
	coutb++;
}
#endif
uint32_t currentBufferIndex = 0;
const uint32_t frameBuffers[] = { FRAME_BUFFER_A, FRAME_BUFFER_B, FRAME_BUFFER_C, FRAME_BUFFER_D };
#define SDRAM_START_ADDR 0xC0000000
#define IMAGE_SIZE (480 * 272 * 2)  // RGB565 ?��?��?���? 480x272 ?��?��?��
uint16_t* image_data = (uint16_t*)SDRAM_START_ADDR;  // SDRAM ?��?�� 주소
...
void chek_all_i2c (void)
{
	for (int i=0;i<backit_len; i++) {
		uint8_t rx_data;

		OV5640_I2C_ReadReg (0x78, backit[i][0], &rx_data, 1);
...
uint32_t* current_frame_buffer = (uint32_t*)FRAME_BUFFER_A;
uint32_t* next_frame_buffer = (uint32_t*)FRAME_BUFFER_B;
uint8_t flag= 0;
...
uint32_t* sdrambuffer = 0xC0000000;
...
void StartDefaultTask(void *argument)
{
  ...
  while(1)
  {
	  ...
	  for (int i=0; i<256; i++) {
		  if (0 == HAL_I2C_Master_Receive (&hi2c1, (uint16_t) i, dummy, 1, 10)) {

		  }

	  }

 	  HAL_I2C_Mem_Read (&hi2c1, 0x78, 0x300A, I2C_MEMADD_SIZE_16BIT, &dummy[0], 1, 10);

	  HAL_I2C_Mem_Read (&hi2c1, 0x78, 0x300B, I2C_MEMADD_SIZE_16BIT, &dummy[1], 1, 10);
